...
<div class="d-flex" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
  ...
  <a href="{{ route('vocab-identification-typeStatus.create') }}" class="btn primary">Nuevo</a>
</div>
        ...
        @forelse($items as $item)
          <tr>
            ...
            <td style="text-align:right;">
              <a class="btn ghost" href="{{ route('vocab-identification-typeStatus.show', $item) }}">Ver</a>
              <a class="btn ghost warn" href="{{ route('vocab-identification-typeStatus.edit', $item) }}">Editar</a>
              <form style="display:inline" method="POST" action="{{ route('vocab-identification-typeStatus.destroy', $item) }}" onsubmit="return confirm('¿Eliminar?')">
                @csrf @method('DELETE')
