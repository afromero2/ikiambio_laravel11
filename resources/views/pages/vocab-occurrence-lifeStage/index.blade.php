...
<div class="d-flex" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
  ...
  <a href="{{ route('vocab-occurrence-lifeStage.create') }}" class="btn primary">Nuevo</a>
</div>
        ...
        @forelse($items as $item)
          <tr>
            ...
            <td style="text-align:right;">
              <a class="btn ghost" href="{{ route('vocab-occurrence-lifeStage.show', $item) }}">Ver</a>
              <a class="btn ghost warn" href="{{ route('vocab-occurrence-lifeStage.edit', $item) }}">Editar</a>
              <form style="display:inline" method="POST" action="{{ route('vocab-occurrence-lifeStage.destroy', $item) }}" onsubmit="return confirm('¿Eliminar?')">
                @csrf @method('DELETE')
